...
var monthsChart = c3.generate({
   ...
   data: {
     ...
     //  y: 'num',
       names: {
           month: 'Month',
           black: 'Black people killed',
           total: 'All people killed'
       },
       type: 'line',
       colors: {
         black: '#9b0b45',
...
var ratesChart = c3.generate({
    ...
    data: {
        ...
        names: {
            race: 'Race',
            rate1mil: 'Rate per million residents, 2015'
        },
        
        color: function(inColor, data) {
            //var cols = ['#0b459b', '#9b0b45', '#459b0b'];
            var cols = ['#810738', '#972308', '#066C2F', '#4F8907'];
...
function charges() {
    var range = ['#888888', '#9b0b45'];
    var palette = d3.scale.ordinal().range(range);

    var tip = d3.tip()
        .attr('class', 'c3-tooltip tool-extra')
        .html(function(d) { 
            return d.name + ': ' + d.num; 
        });

    d3.csv('data/charges.csv', function(dataset) {
        ...
        dataset.forEach(function(d, i) {
            d.num = +d.num;
            d.units = Math.floor(d.num / scale);
            d.type = d.name;
            pts = pts.concat(Array(d.units + 1)
                .join(1).split('')
                .map(function() {
                    return {
                        num: d.num,
...
$(document).ready(function() {
    ...
    $('#ratelink').click(function() {
      ...
      citiesChart.show('rate1mil');
      
    });
    
    $('#numlink').click(function() {
      citiesChart.hide('rate1mil');
